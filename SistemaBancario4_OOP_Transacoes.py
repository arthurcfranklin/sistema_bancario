...
def recuperar_conta_cliente(cliente):
    ...
    # Se o cliente tem mais de uma conta, deixa ele escolher
    print("\nCliente possui mais de uma conta. Escolha qual deseja usar:")
    for i, conta in enumerate(cliente.contas, 1):
        print(f"[{i}] Conta {conta.numero}")

    while True:
        escolha = input("Número da conta: ")
        if escolha.isdigit() and 1 <= int(escolha) <= len(cliente.contas):
...
def criar_conta(clientes, contas):
    ...
    while True:
        ...
        if numero_conta.isdigit():
            ...
            if any(c.numero == numero_conta for c in contas):
                print("\n@@@ Número da conta já existe! Informe outro. @@@")
            else:
                break
        else:
            print("Número inválido, informe apenas dígitos.")
